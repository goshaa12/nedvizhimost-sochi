...
export interface ArendaTorgovyePloshchadiSearchFormProps {
  title?: string;
  description?: string;
  buttonText?: string;
  onButtonClick?: () => void;
...
export function ArendaTorgovyePloshchadiSearchForm({
  title = 'Подберите торговую площадь под бюджет за минуту!',
  description = 'Не тратьте время на неподходящие варианты. Отфильтруйте базу объектов по ключевым параметрам, чтобы сразу увидеть объекты, которые можно рассматривать.',
  buttonText = 'Показать доступные площади',
  onButtonClick,
}: ArendaTorgovyePloshchadiSearchFormProps) {
  return (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        name="withSeparateEntrance"
                        className="w-4 h-4 text-emerald-900 border-gray-300 rounded focus:ring-emerald-900/20"
                      />
                      <span className="text-sm md:text-base text-gray-700">С отдельным входом</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
