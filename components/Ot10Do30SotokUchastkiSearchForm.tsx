...
export interface Ot10Do30SotokUchastkiSearchFormProps {
  title?: string;
  description?: string;
  buttonText?: string;
  onButtonClick?: () => void;
...
export function Ot10Do30SotokUchastkiSearchForm({
  title = 'Подберите участок с учётом всех ваших требований',
  description,
  buttonText = 'Показать результат',
  onButtonClick,
}: Ot10Do30SotokUchastkiSearchFormProps) {
  const [areaFrom, setAreaFrom] = useState('10');
  const [areaTo, setAreaTo] = useState('30');
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="dachnyy">Дачный</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm md:text-base text-gray-700">Подключение света</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-emerald-900 border-gray-300 rounded focus:ring-emerald-900/20"
                      />
                      <span className="text-sm md:text-base text-gray-700">Подключение воды</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 text-emerald-900 border-gray-300 rounded focus:ring-emerald-900/20"
                      />
                      <span className="text-sm md:text-base text-gray-700">Подъезд</span>
                    </label>
                  </div>
                </div>
              </div>
